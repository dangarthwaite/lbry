    ...
    def _try_upnp(self):
        ...
        def add_port_mapping(upnp, port, auto_port, protocol, description):
            if auto_port is True:
                ext_port = upnp.addanyportmapping(port, protocol, upnp.lanaddr, port, description, '')
            else:
                ext_port = upnp.addportmapping(port, protocol, upnp.lanaddr, port, description, '')
            return ext_port
        ...
        def get_port_mapping(upnp, internal_port, auto_port, protocol, description):
            ...
            if external_port is None:
                raise Exception("Can't setup UPnP redirect for port " + str(port) + "(auto=" +
                                str(auto_port) + ")")
        ...
        def threaded_try_upnp():
            ...
            if num_devices_found > 0:
                ...
                if self.peer_port is None:
                    # peer_port not specified by the user: use default and allow upnp to
                    # automatically pick another port if already mapped
                    self.peer_port = conf.settings['DEFAULT_PEER_PORT']
                    auto_port = True
                ...
                auto_port = False
                if self.dht_node_port is None:
                    # dht_node_port not specified by the user: use default and allow upnp to
                    # automatically pick another port if already mapped
                    self.dht_node_port = conf.settings['DEFAULT_DHT_NODE_PORT']
                    auto_port = True
